...
export function dishOrderUrl(chef: Chef, dish: Dish): string {
  const msg = `Hello ${chef.name}! I found you on NextDoorChef and the ${dish.name} caught my eye. I'd love to connect!`
...
export function chefMenuUrl(chef: Chef): string {
  const msg = `Hello ${chef.name}! I found you on NextDoorChef and love what you cook. I'd love to connect!`
  return `https://wa.me/${chef.whatsapp}?text=${encodeURIComponent(msg)}`
}
